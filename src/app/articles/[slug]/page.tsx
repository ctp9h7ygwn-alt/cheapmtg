              ...
              {alternatives.map((swap: any, idx: number) => (
                <div key={swap.oracle_id} className="glass-card rounded-3xl p-6 border border-white/10 space-y-4">
                  ...
                  <div className="pt-3 border-t border-white/5 flex gap-3 justify-end">
                    <a href={swap.tcgplayer_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-gradient-to-r from-amber-500 to-yellow-400 text-black font-extrabold text-xs rounded-xl hover:brightness-110 transition-all flex items-center gap-1 shadow-md shadow-amber-500/20">
                      Buy TCGplayer <ExternalLink className="w-3.5 h-3.5" />
                    </a>
                    <a href={swap.manapool_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-white/5 text-white font-semibold text-xs rounded-xl border border-white/10 hover:bg-white/10 transition-all flex items-center gap-1">
