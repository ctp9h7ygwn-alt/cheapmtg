...
export const metadata: Metadata = {
  title: 'Best Commander Cards Under $1 (Top 100 Cheap EDH Staples) | MTGCheap',
  description:
    'Browse the top 100 best Commander cards under $1.00 in Magic: The Gathering. High-power budget card draw, ramp, removal, board wipes, and lands for penny budgets.',
  keywords: [
    'best commander cards under 1 dollar',
    'best mtg cards under $1',
    'best budget cards under $1',
    'cheap commander cards under a dollar',
    'best cheap cards for commander',
    'edh cards under 1 dollar',
  ],
  alternates: {
    canonical: `${baseUrl}/budget-commander/cards-under-1-dollar`,
  },
  openGraph: {
    title: 'Best Commander Cards Under $1 (Top 100 Budget EDH Staples)',
    description: 'High-power card draw, ramp, removal, and lands for under $1.00.',
    url: `${baseUrl}/budget-commander/cards-under-1-dollar`,
  },
};

async function getCardsUnderOneDollar(): Promise<ClusterCard[]> {
  try {
    const res = await query(
      `SELECT oracle_id, name, mana_value, type_line, oracle_text, price_usd, image_uri, color_identity, colors,
       ...
       FROM cards c
       WHERE price_usd IS NOT NULL AND price_usd > 0 AND price_usd <= 1.00
    ...
    console.error('Error fetching cards under $1:', err);
    return [];
  }
}

export default async function CardsUnderOneDollarPage() {
  const cards = await getCardsUnderOneDollar();

  const jsonLd = [
    {
      '@context': 'https://schema.org',
      '@type': 'Article',
      headline: 'Best Commander Cards Under $1.00 (Ranked by Role)',
      description: 'The definitive list of high-power Commander staples under a single dollar.',
      author: { '@type': 'Organization', name: 'MTGCheap Data Lab' },
      publisher: { '@type': 'Organization', name: 'MTGCheap' },
      datePublished: '2026-08-01',
      dateModified: new Date().toISOString().split('T')[0],
      mainEntityOfPage: `${baseUrl}/budget-commander/cards-under-1-dollar`,
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: 'Cards Under $1', item: `${baseUrl}/budget-commander/cards-under-1-dollar` },
      ],
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-[#f0f6fc] relative selection:bg-amber-500/30 selection:text-amber-200">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12 relative z-10">
        <ClusterNavigation currentSlug="cards-under-1-dollar" />
        ...
        <section className="space-y-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-mono bg-emerald-500/10 text-emerald-300 border border-emerald-500/30">
            <TrendingDown className="w-3.5 h-3.5" /> Programmatic Price Tier
          </div>

          <h1 className="font-cinzel text-3xl sm:text-5xl font-black text-white leading-tight gradient-text-gold">
            Best Commander Cards Under $1.00
          </h1>

          <p className="text-sm sm:text-base text-[#8b949e] leading-relaxed">
            Maximize your deckbuilding budget. These top-tier Magic: The Gathering cards deliver immense power, versatility, and synergy for less than a single dollar.
          </p>
        ...
        <section className="space-y-6">
          <TopicCardExplorer initialCards={cards} roleTitle="Cards Under $1" />
        </section>
      </main>
