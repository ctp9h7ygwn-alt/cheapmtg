...
export const metadata: Metadata = {
  title: 'Best Commander Cards Under $5 (High-Power EDH Staples) | MTGCheap',
  description:
    'Explore the top 100 best Commander cards under $5.00 in Magic: The Gathering. High-impact staples, top tutors, board wipes, and game-winning engines under five dollars.',
  keywords: [
    'best commander cards under 5 dollars',
    'best mtg cards under $5',
    'best budget cards under $5',
    'cheap commander cards under 5 dollars',
    'best cheap cards for commander',
    'edh cards under 5 dollars',
    'commander staples under 5 dollars',
  ],
  alternates: {
    canonical: `${baseUrl}/budget-commander/cards-under-5-dollars`,
  },
  openGraph: {
    title: 'Best Commander Cards Under $5 (High-Power Budget EDH Staples)',
    description: 'High-power card draw, tutors, ramp, removal, and wincons under $5.00.',
    url: `${baseUrl}/budget-commander/cards-under-5-dollars`,
  },
};

async function getCardsUnderFiveDollars(): Promise<ClusterCard[]> {
  try {
    const res = await query(
      `SELECT oracle_id, name, mana_value, type_line, oracle_text, price_usd, image_uri, color_identity, colors,
       ...
       FROM cards c
       WHERE price_usd IS NOT NULL AND price_usd >= 2.00 AND price_usd <= 5.00
    ...
    console.error('Error fetching cards under $5:', err);
    return [];
  }
}

export default async function CardsUnderFiveDollarsPage() {
  const cards = await getCardsUnderFiveDollars();

  const jsonLd = [
    {
      '@context': 'https://schema.org',
      '@type': 'Article',
      headline: 'Best Commander Cards Under $5.00 (Ranked by Role & Power)',
      description: 'The definitive list of high-impact Commander staples under five dollars.',
      author: { '@type': 'Organization', name: 'MTGCheap Data Lab' },
      publisher: { '@type': 'Organization', name: 'MTGCheap' },
      datePublished: '2026-08-01',
      dateModified: new Date().toISOString().split('T')[0],
      mainEntityOfPage: `${baseUrl}/budget-commander/cards-under-5-dollars`,
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: 'Cards Under $5', item: `${baseUrl}/budget-commander/cards-under-5-dollars` },
      ],
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-[#f0f6fc] relative selection:bg-amber-500/30 selection:text-amber-200">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12 relative z-10">
        <ClusterNavigation currentSlug="cards-under-5-dollars" />
        ...
        <section className="space-y-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-mono bg-amber-500/10 text-amber-300 border border-amber-500/30">
            <TrendingDown className="w-3.5 h-3.5" /> High-Power Budget Tier
          </div>

          <h1 className="font-cinzel text-3xl sm:text-5xl font-black text-white leading-tight gradient-text-gold">
            Best Commander Cards Under $5.00
          </h1>

          <p className="text-sm sm:text-base text-[#8b949e] leading-relaxed">
            The $5 price cap unlocks elite tournament-grade power: Diabolic Intent, Growing Rites of Itlimoc, Ashnod&apos;s Altar, Heroic Intervention alternatives, and efficient pain lands. Build high-synergy decks without $50+ staples.
          </p>
        ...
        <section className="space-y-6">
          <TopicCardExplorer initialCards={cards} roleTitle="Cards Under $5" />
        </section>
      </main>
